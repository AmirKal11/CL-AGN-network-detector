...
> **Note on what this backbone is — and isn't.** The Type 1/2 classifier in §3.1–3.2 is an **architecture validation**, not the final pipeline. It was trained on a supervised classification task to confirm that the encoder architecture has the capacity to learn physically meaningful spectral features. The classifier itself was discarded; what was kept is the *architecture* (`SpectraNet` → `SpectraEncoder`), which was then repurposed as the encoder in the self-supervised Stage 1. The masking ablation (§3.2) demonstrates that the architecture *can* read the emission-line physics — not that the classifier is part of the CL-AGN detector.
...
models/
  ...
  v1_backbone/              # Type 1/2 architecture validation (§3 confusion matrices)
  clagn_v2_baseline/        # baseline SSL encoder (DR7+DESI, 44k)
  clagn_v2_extended/        # Phase A full retrain (+DR16+SDSS-V, 53k) — superseded by v4
  clagn_v2_continual/       # Phase B continual + replay
  clagn_v4/                 # v4 confound-free, recall-first rebuild (§7)
  # --- v2 ablation runs (all: DR16×SDSS-V pairs, uncapped full DR7 SSL pool) ---
  raw_continuum_full_dr7/   # ch0 = raw MAD only (no continuum subtraction) — PR-AUC 0.812
  fixed_OIII/               # ch0 = continuum-subtracted MAD — PR-AUC 0.832, best result
  sdssv_weighted/           # fixed_OIII + SDSS-V/DR16 ×3 survey-weighted SSL loss — TBD
