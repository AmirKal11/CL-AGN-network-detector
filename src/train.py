...
def process_spectra_robust(x, kernel_size=5):
    """
    1. Smooths noise
    2. Takes derivative to drop continuum
    3. Compresses amplitude
    """
    # 1. Mild Smoothing
    padding = kernel_size // 2
    x_padded = F.pad(x, (padding, padding), mode='replicate')
    x_smoothed = F.avg_pool1d(x_padded, kernel_size=kernel_size, stride=1)
    
    # 2. Derivative
    diff = torch.diff(x_smoothed, dim=-1)
    
    # 3. Pad to maintain 1024 length
    diff_padded = F.pad(diff, (1, 0), mode='replicate')
    
    # 4. Compression
    diff_compressed = torch.sign(diff_padded) * torch.log1p(torch.abs(diff_padded))
...
def train_one_epoch(model, dataloader, criterion_class, criterion_redshift, optimizer, device, alpha,lambda_z = 0.0):
    ...
    for batch_X, batch_y, batch_z in dataloader:
        ...
        batch_X = morphological_continuum_subtraction(batch_X, window_size=151)
        optimizer.zero_grad()
...
def train_model():
    ...
    criterion_class = BinaryFocalLossWithLogits(alpha=0.85, gamma=2.0)
    criterion_redshift = nn.HuberLoss() 
    optimizer = optim.AdamW(model.parameters(), lr=config['training']['learning_rate'], weight_decay=float(config['training']['weight_decay']))
    ...
    for epoch in range(config['training']['num_epochs']):
        ...
        if epoch < warmup_epochs:
            ...
            print(f"Epoch {epoch+1} | Phase: Warm-up (Adversary OFF)")
        else:
            current_lambda_z = max_lambda * alpha
            print(f"Epoch {epoch+1} | Phase: Adversarial (Adversary ON)")        
